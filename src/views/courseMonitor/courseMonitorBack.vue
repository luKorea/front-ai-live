  ...
  <basic-container>
    <avue-crud :option="option"
      ...
      <template slot="menuLeft">
        <el-button type="primary" size="small" @click='monitorUser'>查看课程监控
        </el-button>
      </template>
    </avue-crud>
            ...
            <el-card class="box-card" shadow="never">
              <el-row :gutter="20">
                ...
                <el-col :span="4">
                  ...
                  <el-button type="primary" @click="sendMessage">普通发送
                  </el-button>
                </el-col>
              </el-row>
            </el-card>
...
export default {
  data() {
    return {
      ...
      loading: true,
      wsuri: "ws://192.168.0.107:80/websocket/admin",
      page: {
        pageSize: 10,
        currentPage: 1,
        total: 0
  ...
  methods: {
    ...
    connect() {
      ...
      this.stompClient.connect({"userId": this.userId}, this.onConnected, this.onError)
    },
    // 加入请求
    onConnected(frame) {
      ...
      this.stompClient.send("/app/chat.addUser",
        {},
        JSON.stringify({sender: 'admin', type: 'JOIN'})
      )
    },
    // 错误处理
    onError(error) {
    ...
    sendMessage(event) {
      const _this = this;
      var messageContent = this.messageInfo;
    ...
    monitorUser(row) {
      ...
      // this.$router.push({
      //   path: '/courseMonitor/monitor',
      //   params: {
      //     data: this.selectionList[0] || {}
      //   }
      // })
      this.showModal = true;
      this.connect();
